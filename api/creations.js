...
const getObjkts = async () => {
  return await axios
    .get(
      `https://api.better-call.dev/v1/contract/mainnet/KT1RJ6PbjHpwc3M5rw5s2Nbmefwbuwbdxton/tokens`
    )
    .then((res) => res.data);
};
...
const owners = async (obj) => {
  ...
  obj.total_amount =
    values_arr.map((e) => parseInt(e)).length > 0
      ? values_arr.filter((e) => parseInt(e) > 0).reduce(reducer)
      : 0;
  obj.owners = owners;
  console.log(obj);
...
const totalAmountIntegral = async (obj) => {
  ...
  obj.total_amount =
    values_arr.map((e) => parseInt(e)).length > 0
      ? values_arr.filter((e) => parseInt(e))
      : 0;
...
const getObjktLedger = async () =>
  await axios
    .get("https://better-call.dev/v1/bigmap/mainnet/511/keys?size=6500")
    .then((res) =>
      res.data.map((e) => ({
        amount: parseInt(e.data.value.value),
        tz: e.data.key.children[0].value,
        tk_id: parseInt(e.data.key.children[1].value),
      }))
    );
const gethDAOLedger = async () =>
  await axios
    .get("https://api.better-call.dev/v1/bigmap/mainnet/519/keys?size=4000")
    .then((res) =>
      res.data.map((e) => {
        return {
          token_id: parseInt(e.data.key.value),
          hDAO_balance: parseInt(e.data.value.children[0].value),
        };
      })
    );
...
const getSwaps = async () => {
  return await axios
    .get(`https://api.better-call.dev/v1/bigmap/mainnet/523/keys?size=6000`)
    .then((res) => {
      return res.data.map((e) => {
        ...
        e.data.value != null
          ? e.data.value.children.map((e) => (obj[e.name] = e.value))
          : null;
        return obj;
      });
    });
...
const getTzCreations = async (tz, res) => {
  /*     var ledger = desc(await getObjktLedger())
        var objkts = await getObjkts()
        var tzLedger = _.map(filterTz(ledger, tz), (obj) => _.assign(obj, _.find(objkts, { token_id : obj.tk_id })))
     */
  ...
  var result = arr.map(async (e) => {
    ...
    e.token_info = await getObjktById(e.token_id);
...
module.exports = allowCors(async (req, res) => {
  await getTzCreations(req.body.tz, res);
});
